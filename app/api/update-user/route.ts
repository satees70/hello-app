...
export async function POST(request: Request) {
  const { id, username, full_name, factory_code, factory_codes, readonly_factories, role, permissions, password } = await request.json()
  if (!id) return NextResponse.json({ error: 'Missing user id' }, { status: 400 })

  const profileUpdate: Record<string, unknown> = { full_name, factory_code, factory_codes: factory_codes ?? [factory_code], readonly_factories: readonly_factories ?? [], role, permissions: permissions ?? {} }
