...
export async function POST(request: Request) {
  const { username, email, password, full_name, factory_code, factory_codes, readonly_factories, warehouse_user, role, permissions, capabilities, location_perms, customer_filter } = await request.json()
  ...
  const { error: profileError } = await supabaseAdmin
    .from('profiles')
    .insert({ id: authData.user.id, username: uname, email: email || loginEmail, full_name, factory_code, factory_codes: factory_codes ?? [factory_code], readonly_factories: readonly_factories ?? [], warehouse_user: !!warehouse_user, role, permissions: permissions ?? {}, capabilities: capabilities ?? {}, location_perms: location_perms ?? {}, customer_filter: (customer_filter ?? '').trim() || null })
  if (profileError) {
