...
export async function POST(request: Request) {
  const { username, email, password, full_name, factory_code, factory_codes, readonly_factories, warehouse_user, role, permissions } = await request.json()
  ...
  const { error: profileError } = await supabaseAdmin
    .from('profiles')
    .insert({ id: authData.user.id, username: uname, email: email || loginEmail, full_name, factory_code, factory_codes: factory_codes ?? [factory_code], readonly_factories: readonly_factories ?? [], warehouse_user: !!warehouse_user, role, permissions: permissions ?? {} })
  if (profileError) {
