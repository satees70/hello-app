...
interface PackLine { id: string; factory_code: string; name: string; active: boolean }
...
export default function PackingLinesPage() {
  ...
  async function addLine(e: React.FormEvent) {
    ...
    const { error: e1 } = await supabase.from('packing_lines').insert({ factory_code: factory, name: name.trim() })
    setSaving(false)
    if (e1) { setError(e1.message.includes('duplicate') ? 'That line already exists for this factory.' : e1.message); return }
    setName(''); setSuccess('Packing line added.'); load()
  }
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {shown.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-400">No packing lines yet.</td></tr>}
              {shown.map(l => (
