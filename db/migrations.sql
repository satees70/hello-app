...
-- ============================================================================
-- 2026-06 · GCH orders: visible to EVERYONE, kept OPEN until packing is chosen.
-- GCH lines come in with a blank factory (location GCH-105 is intentionally NOT
-- mapped) so they stay general/open until someone decides who packs each item.
-- The problem was only visibility: non-HO users (e.g. Gopi) couldn't see blank-
-- factory lines. These permissive policies let ANY logged-in user view GCH
-- orders (lines + documents) regardless of factory, so they can pick the
-- packing factory via the normal "resolve / assign" flow. Customer-restricted
-- users (e.g. sem118 = GCH-only) are unaffected: their restrictive policy still
-- limits them, and GCH passes it.
-- NOTE: GCH-105 is deliberately left out of location_map and lines are NOT
-- auto-assigned a factory — keep them open.
-- ============================================================================
...
-- ============================================================================
-- 2026-06 · Keep the uploaded delivery files (so the schedule can be amended).
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- All new tables live in this one Supabase project alongside the portal.
-- Timezone is Asia/Kuala_Lumpur everywhere: punch_time is stored as timestamptz
-- (ZKLink's "yyyy-MM-dd HH:mm:ss" is interpreted as KL local time on ingest),
-- and work_date is the KL calendar date the punch belongs to.
-- RLS is permissive for now (placeholder-auth phase) like delivery_trips above;
-- it gets locked down when real Supabase Auth replaces lib/auth.ts before launch.
-- Tables are created parent-first so foreign keys resolve.
-- ============================================================================

-- Shift profiles — per-employee OT threshold + how lunch is handled.
-- lunch_rule 'punch'      = use the employee's real lunch punches.
-- lunch_rule 'auto_deduct'= deduct lunch_minutes IF a lunch break is confirmed;
--   never deducted silently — a long no-lunch session is flagged needs_review.
...
-- Punches have NO in/out tag and NO unique id, so source_id is our dedupe key:
-- 'employee_code|punch_time'. Pairing (IN/OUT) is computed later, not stored here.
...
-- Written when a day is flagged needs_review (missing lunch punch / odd punch
-- count / missing clock-out). One row per employee per day.
...
-- Sync cursor — remembers how far ZKLink has been pulled. Sync filters by DAY,
-- so we re-pull from last_synced_date and let source_id dedupe the overlap.
...
-- Drivers — delivery drivers. employee_code optionally links to employees so the
-- productivity join (attendance ↔ deliveries) works once both modules are live.
...
-- Deliveries — one stop on a driver's day. gate_device_id (when set) shows the
-- "Open gate" button (Tuya). Photo + odometer captured on mark-delivered.
...
-- ============================================================================
-- 2026-07 · EASWARI Driver app — wired to the EXISTING delivery schedule.
-- ----------------------------------------------------------------------------
-- The driver app reads delivery_resources (drivers) + delivery_trips (a driver's
-- lines for a day) + delivery_schedule (the SO orders = stops). These columns let
-- a driver mark each order delivered (with a proof photo + note) and record the
-- odometer once per line/trip. The earlier `drivers`/`deliveries` tables are now
-- unused (superseded by the delivery schedule) and can be dropped later.
-- ============================================================================
alter table public.delivery_schedule add column if not exists delivered_at timestamptz;
alter table public.delivery_schedule add column if not exists delivery_photo_path text;
alter table public.delivery_schedule add column if not exists delivery_note text;
alter table public.delivery_trips add column if not exists odometer_start int;
alter table public.delivery_trips add column if not exists odometer_end int;

-- 2026-07 · Shift profiles get a start/end window + early-OT cutoff (as 'HH:MM').
-- Window OT: normal = work inside [shift_start, shift_end] minus lunch; OT = work
-- before ot_before + work after (shift_end + 15min grace), rounded to nearest 15m.
-- Late / early-out are measured against shift_start / shift_end. Profiles without
-- a window fall back to the flat "over normal_hours" rule.
alter table public.shift_profiles add column if not exists shift_start text;
alter table public.shift_profiles add column if not exists shift_end text;
alter table public.shift_profiles add column if not exists ot_before text;
...
alter table public.shift_profiles add column if not exists week_schedule jsonb;
...
drop policy if exists ph_read on public.public_holidays;
create policy ph_read on public.public_holidays for select using (true);
drop policy if exists ph_write on public.public_holidays;
create policy ph_write on public.public_holidays for all using (true) with check (true);
